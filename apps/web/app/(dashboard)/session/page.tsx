...
function SessionSidebar({
  ...
  return (
    <div className="w-60 shrink-0 border-r border-[rgba(255,255,255,0.05)] flex flex-col bg-[#0f1011] h-full">
      {/* DMs section */}
      <div className="px-3 pt-4 pb-1">
        <h3 className="text-xs font-semibold text-[#62666d] uppercase tracking-wider flex items-center gap-1.5">
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                {(conv.unread_count ?? 0) > 0 && (
                  <span className="shrink-0 text-[10px] bg-[#5e6ad2] text-white rounded-full px-1.5 py-0.5 leading-none">
                    {conv.unread_count}
                  </span>
                )}
              </div>
              {conv.last_message && (
                <div className="text-xs text-[#8a8f98] truncate mt-0.5">
...
function InboxPanel({
  ...
  return (
    <div className="w-72 shrink-0 border-l border-[rgba(255,255,255,0.05)] flex flex-col h-full bg-[#191a1b]">
      <div className="flex items-center justify-between px-4 h-12 border-b border-[rgba(255,255,255,0.05)] shrink-0">
        <h3 className="text-sm font-semibold text-[#f7f8f8]">通知</h3>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-auto min-h-0">
        {loading ? (
          ...
          items.map((item) => (
            <div
              key={item.id}
              className="flex items-start gap-2 px-3 py-2.5 border-b border-[rgba(255,255,255,0.05)] hover:bg-[rgba(255,255,255,0.05)] transition-colors group"
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`text-sm truncate ${!item.read ? "font-medium text-[#f7f8f8]" : "text-[#8a8f98]"}`}
                  >
                    {item.title}
                  </span>
                </div>
              ...
              <button
                ...
                className="hidden group-hover:block shrink-0 text-[#8a8f98] hover:text-[#f7f8f8] p-0.5"
              >
...
export default function SessionPage() {
  ...
  return (
    <div className="flex h-full">
      ...
      {/* Center - message area */}
      <div className="flex-1 flex flex-col min-w-0 h-full bg-[#08090a]">
        {selectedId ? (
          <>
            {/* Header */}
            <div className="flex items-center justify-between px-4 h-12 border-b border-[rgba(255,255,255,0.05)] shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                {selectedType === "channel" ? (
                  <Hash className="h-4 w-4 shrink-0 text-[#8a8f98]" />
                ) : (
                  <MessageCircle className="h-4 w-4 shrink-0 text-[#8a8f98]" />
                )}
                <span className="font-medium text-sm truncate text-[#f7f8f8]">
                  {headerName}
                </span>
                {headerSub && (
                  <>
              ...
              <button
                ...
                className="relative p-1.5 rounded-md hover:bg-[rgba(255,255,255,0.05)] text-[#8a8f98] hover:text-[#f7f8f8] transition-colors"
                title="通知"
              >
                {inboxUnread > 0 ? (
                ...
                {inboxUnread > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 text-[9px] bg-[#5e6ad2] text-white rounded-full h-3.5 w-3.5 flex items-center justify-center leading-none">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-[#8a8f98] gap-2">
            <MessageCircle className="h-10 w-10 text-[#62666d]" />
