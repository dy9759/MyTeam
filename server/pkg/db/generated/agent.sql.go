...
func (q *Queries) ListCloudPendingTasks(ctx context.Context) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listCloudPendingTasks)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) SetAgentScope(ctx context.Context, arg SetAgentScopeParams) error {
	_, err := q.db.Exec(ctx, setAgentScope, arg.ID, arg.Scope, arg.PageScope)
	return err
}
