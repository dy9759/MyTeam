...
// checked out on demand by the agent via `myteam repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string           // base path for all envs (e.g., ~/myteam_workspaces)
...
// on demand via `myteam repo checkout <url>`.
