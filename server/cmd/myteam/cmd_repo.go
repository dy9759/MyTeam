...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := cli.EnvAny("MYTEAM_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("MYTEAM_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := cli.EnvAny("MYTEAM_WORKSPACE_ID")
	agentName := cli.EnvAny("MYTEAM_AGENT_NAME")
	taskID := cli.EnvAny("MYTEAM_TASK_ID")
