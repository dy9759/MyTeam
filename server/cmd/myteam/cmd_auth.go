...
import (
	...
	"github.com/dy9759/MyTeam/server/internal/auth"
...
var authLoginCmd = &cobra.Command{
	Use:    "login",
	Short:  "Authenticate with MyTeam",
	Long:   "Authenticate with MyTeam without auto-configuring workspaces. Use 'myteam login' for the guided setup flow.",
	Hidden: true,
	RunE:   runAuthLogin,
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := cli.EnvAny("MYTEAM_TOKEN"); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"MYTEAM_APP_URL", "FRONTEND_ORIGIN"} {
...
func runAuthLoginToken(cmd *cobra.Command) error {
	...
	if err := validatePersonalAccessToken(token); err != nil {
		return err
	}
...
func validatePersonalAccessToken(token string) error {
	if token == "" {
		return fmt.Errorf("token is required")
	}
	if !auth.IsPATToken(token) {
		return fmt.Errorf("invalid token format: must start with %s", auth.PersonalAccessTokenPrefix)
	}
	return nil
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'myteam login' to authenticate.")
		return nil
	}
...
<title>MyTeam — Authenticated</title>
<style>
