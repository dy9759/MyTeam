...
import (
	...
	"github.com/MyAIOSHub/MyTeam/server/internal/daemon"
	logger_pkg "github.com/MyAIOSHub/MyTeam/server/internal/logger"
)
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: MYTEAM_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: MYTEAM_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: MYTEAM_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: MYTEAM_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: MYTEAM_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Per-task timeout (env: MYTEAM_AGENT_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: MYTEAM_DAEMON_MAX_CONCURRENT_TASKS)")
...
// Empty profile → ~/.myteam/, named profile → ~/.myteam/profiles/<name>/.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnvAny(cmd, "server-url", []string{"MYTEAM_SERVER_URL"}, "")
	if serverURL == "" {
