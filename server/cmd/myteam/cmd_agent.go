...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, MYTEAM_SERVER_URL env, or 'myteam config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := cli.EnvAny("MYTEAM_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := cli.EnvAny("MYTEAM_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnvAny(cmd, "server-url", []string{"MYTEAM_SERVER_URL"}, "")
	if val != "" {
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnvAny(cmd, "workspace-id", []string{"MYTEAM_WORKSPACE_ID"}, "")
	if val != "" {
		return val
	}
	profile := resolveProfile(cmd)
