...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers MYTEAM_APP_URL", func(t *testing.T) {
		t.Setenv("MYTEAM_APP_URL", "http://localhost:15000")
		t.Setenv("FRONTEND_ORIGIN", "http://localhost:13000")

		if got := resolveAppURL(cmd); got != "http://localhost:15000" {
			t.Fatalf("resolveAppURL() = %q, want %q", got, "http://localhost:15000")
		}
	})

	t.Run("falls back to FRONTEND_ORIGIN", func(t *testing.T) {
		t.Setenv("MYTEAM_APP_URL", "")
	...
	t.Run("defaults to production", func(t *testing.T) {
		...
		if got := resolveAppURL(cmd); got != "https://myteam.ai" {
			t.Fatalf("resolveAppURL() = %q, want %q", got, "https://myteam.ai")
		}
	})
}
...
func TestResolveServerURL(t *testing.T) {
	...
	t.Run("prefers MYTEAM_SERVER_URL", func(t *testing.T) {
		t.Setenv("MYTEAM_SERVER_URL", "ws://localhost:19090/ws")

		if got := resolveServerURL(cmd); got != "http://localhost:19090" {
			t.Fatalf("resolveServerURL() = %q, want %q", got, "http://localhost:19090")
		}
	})

	t.Run("defaults to myteam api", func(t *testing.T) {
		t.Setenv("MYTEAM_SERVER_URL", "")
		t.Setenv("HOME", t.TempDir())

		if got := resolveServerURL(cmd); got != "https://api.myteam.ai" {
			t.Fatalf("resolveServerURL() = %q, want %q", got, "https://api.myteam.ai")
		}
	})
}
